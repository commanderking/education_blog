...
2. In one problem, we offer three opportunities for students to apply the Pythagorean theorem. This gives us some flexibility in how we use this problem. A teacher could for example, follow a "I do, we do, you do" approach in which they model with one household, get student volunteers to work together on computing another problem, and ask students to determine the last problem.
...
4. There is some room for students to approach this problem differently in meaningful ways. For example, the blind approach to this problem would be for a student to calculate the distance between every household and the cell tower. But some students might first estimate the distance of each household to the cell tower. That might help them determine certain households are already out of range and do not need a calclulation.

Here's what I didn't quite like:

1. Ultimately, the primary work is still performing a calculation and at most reaches the "analyze" level of Bloom's taxonomy. There's some critical thinking for students who take a creative approach. While not every lesson or problem can invoke the highest tiers of Bloom's taxonomy or require high levels of critical thinking, I wanted to see if we could make some small adjustments that could help it require students to evaluate or create.
