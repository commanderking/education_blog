...
In <a href="./cell-tower-1" target="_blank">Part 1</a>, I was inspired by culturally responsive teaching to create a lesson around the Pythagorean theorem that would:

1. Encourage students to find different valid solutions to a realistic problem.
1. Help students see that applying math can help people make better decisions that have an impact on many people.
...
I can see the above questions being effective for a quick check of student understanding at the end of a class to see whether students have understood how to apply the formula. How quickly these problems can be solved and assessed is likely why they're valuable, but if a teacher were to only provide students with questions like these throughout the year, it's fairly easy to see why the majority of students would not want to pursue math or understand how learning math could be fun or meaningful.
...
This problem feels slightly better than the IXL word example, but only marginally. While students can better relate to the frustration of not having cell service, they'll still rightly question why a person couldn't just test the cell service when at the new house location. Students really need to forget reality in order to make sense of the problem at hand.

When I would design my own lessons, I often fell into the trap of creating word problems like the one above. I thought if I could add some text that might connect it to a student interest, they might be more interested in learning. For example, if a student liked baseball, maybe we'd calculate the distance from first base to third base. But in reality, I found that the more a student knew about a topic, the more the context of the problem would actually make less sense. In the baseball example, no one actually runs from first to third base. And if we could measure the distance between the other bases, why couldn't we just measure across there?
